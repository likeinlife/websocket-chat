...
@dataclass
class BaseMediator(tp.Generic[EVENT, HANDLER_RETURN, MEDIATOR_RETURN]):
    handler_map: dict[type[EVENT], list[IMediatorHandler[EVENT, HANDLER_RETURN]]] = field(
    ...
    def register(self, event: type[EVENT], handler: IMediatorHandler[EVENT, HANDLER_RETURN]) -> None:
        ...
        if command_handler_type != event:
            raise HandlerEntityTypeError(event, command_handler_type, handler)
